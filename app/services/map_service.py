...
class MapService:
    ...
    def create_empty_map_placeholder(self, mission_count: int = 0):
        """
        Create a placeholder when no missions have coordinates.
        
        Args:
            mission_count: Number of missions without coordinates
            
        Returns:
            ft.Container with placeholder content
        """
        try:
            import flet as ft
        except ImportError:
        ...
        return ft.Container(
            ft.Column([
                ft.Icon(ft.Icons.MAP_OUTLINED, size=50, color=ft.Colors.GREY_400),
                ft.Text(
                    f"{mission_count} missions" if mission_count > 0 else "No missions yet",
                    size=14,
                    color=ft.Colors.BLACK54,
                    weight="w500"
                ),
                ft.Text(
                    "Locations will appear when geocoded",
                    size=11,
